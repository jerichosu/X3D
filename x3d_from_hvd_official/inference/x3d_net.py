...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 22 18:19:24 2021
...
class ResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 dropout=0.5,
                 n_classes=400,
                 base_bn_splits=8, # this controls the batch size!!!!!!!!!!!!!!!!!!
                 task='class'):
        ...
        self.base_bn_splits = base_bn_splits # 1
        self.task = task # string: 'class'
        ...
        self.bn1 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=self.in_planes, affine=True) #nn.BatchNorm3d(self.in_planes) #24
        self.relu = nn.ReLU(inplace=True)
        # -> 16x112x112
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride):
        ...
        layers.append(
            block(in_planes=self.in_planes, #24
                  ...
                  index=self.index, # 0
                  base_bn_splits=self.base_bn_splits)) # 1
...
if __name__ == "__main__":
    ...
    input_tensor = torch.autograd.Variable(torch.rand(1, 3, 16, 224, 224)) # [B (base_bn_splits), C, T, W, H]
    
    x3d_version = 'M'
    model = generate_model(x3d_version, base_bn_splits=1)
    ...
    # model_path = 'models/fb_pretrained_400.pt'
