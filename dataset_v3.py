...
class VideoDataset(Dataset):
    ...
    def __init__(self, directory, mode, clip_len, alpha, tau, transform = None,
        ...
        # training transfomration
        self.trans=T.Compose([T.RandomResizedCrop(size = 224, scale = (0.8,1.0)),
                     T.RandomHorizontalFlip()])
        
        # testing/validation transformation
        self.trans_val=T.Compose([T.Resize(size = [224, 224]),
                             ])
...
# -----------------------------------------------------------------------------
    ...
    def __getitem__(self, index):
        ...
        if self.transform == 'train':
            video_data = self.trans(video_data)
            
        if self.transform == 'val':
            video_data = self.trans_val(video_data)
...
if __name__ == '__main__':
    ...
    '''
    'trans': for training transformation
    'val': for testing transformation
    '''
    ...
    train_dataloader = \
        DataLoader( VideoDataset(datapath, mode='test', clip_len= 128, alpha = 8, tau = 64, transform = 'train'), batch_size=16, shuffle=True, num_workers=4)
